...
{
    public class DeleteGroupRequestHandler : IRequestHandler
    {
        private readonly DeleteGroupsRequest request;
        ...
        public async Task<Response> CreateResponseAsync()
        {
            try
            {
                await groupsService.RemoveUserGroupsAsync(request.GroupsId, clientConnection.UserId.GetValueOrDefault()).ConfigureAwait(false);
                return new ResultResponse(request.RequestId);
            }
            catch (ObjectDoesNotExistsException)
            {
                return new ResultResponse(request.RequestId, "Groups not found.", ObjectsLibrary.Enums.ErrorCode.ObjectDoesNotExists);
            }            
        }
        ...
        public bool IsRequestValid()
        {
            ...
            return request.GroupsId != null && request.GroupsId.Any();
        }
    }
}